...
/* known information */
	...
	for(int i=0;i<7;i++){
		obstacles.push_back({layerpts[(i+3)%7],layerpts[i]});
		obstacles.push_back({                8,layerpts[i]});
		obstacles.push_back({layerpts[(i+4)%7],layerpts[i]});
		obstacles.push_back({layerpts[(i+4)%7],8          });
	}
	for(int i=0,j,u=9,v;i<partition.size();i++,u=v){
		j=(i+1)%partition.size();
		v=u+partition[i];
		for(int w=u;w<v;w++){
			new_known( idx[obstacles[i].first][obstacles[i].second][w]);
			new_known(-idx[obstacles[j].first][obstacles[j].second][w]);
		} 
	}
/* ----------------- */
...
void mk_no6hole_710_outside_28parts(int m,vector<int> partition){
	int n=7+1+m;
	
	string s="no-6hole-710-outside-"+to_string(m)+"(";
	for(int i=0;i<partition.size();i++) s.push_back(partition[i]<10?'0'+partition[i]:'a'+(partition[i]-10));
	s=s+").sat";
	fout.open(s);
	fout<<"c\n";
	//fout<<"p cnf 123 0\n"; 
...
int main(int argc, char *argv[]){
	if(argv[1][0]=='-'){
		...
		if(argc==3){
			...
			if(len!=28) printf("Length of the given partition is %d != 28.\n", len);
			...
			for(int i=0;i<28;i++){
				if(i>=len){
					...
					int b;
					if('0'<=argv[2][i] && argv[2][i]<='9') b=argv[2][i]-'0';
					else                                   b=argv[2][i]-'a'+10;
					if(cnt+b>n){
						printf("The sum of numbers in the given partition is greater than n=%d.\n", n);
						b=n-cnt;
					}
			...
			if(cnt<n){
				printf("The sum of numbers in the given partition is smaller than n=%d.\n", n);
				partition[0]+=n-cnt;
			}
			
			printf("n = %d.\n", n);
			printf("partition = {%d",partition[0]);
			for(int i=1;i<28;i++) printf(",%d",partition[i]);
			printf("}.\n");
			
			mk_no6hole_710_outside_28parts(n,partition);
		}
	}
} 
