...
void cc_system(int n,int m,vector<int> secondhullst){
	for(int i=1;i<=n;i++)
		for(int j=i+1;j<=n;j++)
			for(int k=j+1;k<=n;k++){
				int x=new_var();
...
/* known information */
	...
	// make them apart based on the second hull structure
	for(int i=9,i2,j=0;j<secondhullst.size();i=i2,j++){ 
		if(j>=1 && secondhullst[j-1]==3 && secondhullst[j]>=2){ // Symmetry Breaking for Triangles
			int p=i-3,q=i-2,x=i,y=i+1;
			new_known(idx[p][y][x]); new_known(idx[q][x][y]);
		} 
		if(j>=1 && secondhullst[j-1]==4){ // Symmetry Breaking for Quadrilaterals
			int p=i-4,q=i-3,r=i-2,s=i-1;
			new_known(idx[q][s][i]); new_known(idx[r][p][i]);
		}
		if(j>=1 && secondhullst[j-1]==5){ // Symmetry Breaking for Pentagons
			int p=i-5,q=i-4,r=i-3,s=i-2,t=i-1;
			new_known(idx[q][s][i]); new_known(idx[s][p][i]); new_known(idx[r][t][i]);
			new_clauses({idx[q][t][i],idx[p][r][i]}); 
		} 
		if(j>=1 && secondhullst[j-1]==6){ // Symmetry Breaking for Hexagons
			int p=i-6,q=i-5,r=i-4,s=i-3,t=i-2,u=i-1;
			new_known(idx[q][t][i]); new_known(idx[s][p][i]);
		}
...
void mk_no6hole_710_outside_secondhullst(int m,vector<int> secondhullst){
	int n=7+1+m;
	
	string s="no-6hole-710-outside-"+to_string(m)+"{";
	for(int i=0;i<secondhullst.size();i++) s.push_back('0'+secondhullst[i]);
	s=s+"}.sat";
	fout.open(s);
	fout<<"c\n";
	//fout<<"p cnf 123 0\n"; 
...
int main(int argc, char *argv[]){
	if(argv[1][0]=='-'){
		...
		if(argc==3){
			...
			if(cnt!=n){
				printf("The total size of the second hull structure = %d, but n = %d.\n",cnt,n);
				exit(1);
			}
			...
			mk_no6hole_710_outside_secondhullst(n,secondhullst);
		}
	}
} 
