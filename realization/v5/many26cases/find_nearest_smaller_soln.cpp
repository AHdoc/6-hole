...
int main(int argc, char *argv[]) {
	...
	if (config.start_log_filename != "") {
		...
		logger << "progress loaded from " << config.start_log_filename << " @ " << Logger::endl;
	}
	...
	for(int i=0;i<pts.size();i++){
		...
		if(pt.size()==P){
			...
			for(int j=0;j<i;j++){
				...
				if(qt.size()==Q){
					...
					for(int u=0;u<P;u++){
						...
						for(int v=0;v<Q;v++){
							double Dist=sqrt((pt[u].first-qt[v].first)*(pt[u].first-qt[v].first)+(pt[u].second-qt[v].second)*(pt[u].second-qt[v].second));
							if(closestDist<0.0 || Dist<closestDist)
								closestDist=Dist;
						}
						Dists.push_back(closestDist);
					}
					sort(Dists.begin(),Dists.end());
					double sumDist=0.0;
					for(int u=0;u<P/3*2;u++)
						sumDist+=Dists[u];
					if(minSumDist<0.0 || sumDist<minSumDist){
						minSumDist=sumDist;
						bestj=j; 
					}
				}
			...
			sort(Dists.begin(),Dists.end());
			for(auto x:Dists) cerr<<fixed<<x<<" ";
			cerr<<"\n";
			//cerr<<Q<<":";
			//for(int u=0;u<Q;u++)
			//	cerr<<"("<<qt[u].first<<","<<qt[u].second<<")";
			//cerr<<"\n->\n"<<P<<":";
			//for(int u=0;u<P;u++)
			//	cerr<<"("<<pt[u].first<<","<<pt[u].second<<")";
			//cerr<<"\n\n";
		}
	}
}
