...
namespace geo {
	...
	double Distance_Point_to_Line(P p,Line L){
		return Cross(L.v,p-L.p)/Length(L.v);
	}
	double Distance_InteriorPoint_to_Polygon(P p,vector<P> q){
		double ret=Distance_Point_to_Line(p,Line(q[q.size()-1],q[0]));
		for(int i=0;i+1<q.size();i++){
			double now=Distance_Point_to_Line(p,Line(q[i],q[i+1]));
			if(now<ret) ret=now;
		}
		return ret;
	}
...
LL dfs(int i,int ii,vector<pair<LL,LL>> pt){ // points numbered from ii to i are of the same level
	...
	if(area<geo::eps) return max_depth;
	...
	for(LL t1=1,t2=1;t1<=base && t2<=multi*base;t2++){
		...
		if(!check(i,ii,pt,p)){
			...
			pt2=pt;
			pt2.push_back(p);
			now_depth=dfs(i+1,lvl[i]==lvl[i+1]?ii:i+1,pt2);
		}
		
		LL pace=geo::Distance_InteriorPoint_to_Polygon(_p,range)*0.5;
		int prev_k=5;
		while(pace>=1){
			LL depth;
			LL dirx[4]={0,-pace,pace,0},diry[4]={pace,0,0,-pace};
			for(int k=0;k<=4;k++){
				if(k+prev_k==3) continue;
				if(k==4){
					pace/=2;
					break;
				}
				p2=make_pair(p.x+dirx[k],p.y+diry[k]);
				if(!check(i,ii,pt,p2)) continue;
				else{
					pt2=pt;
					pt2.push_back(p2);
					depth=dfs(i+1,lvl[i]==lvl[i+1]?ii:i+1,pt2);
				}
				if(depth>now_depth){
					prev_k=k;
					p=p2;
